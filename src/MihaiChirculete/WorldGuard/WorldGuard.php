...
class WorldGuard extends PluginBase
{
    ...
    public function getRegionNameFromPosition(Position $pos): string
    {
        ...
        foreach ($this->regions as $name => $region) {
            if ($region->getLevelName() === $pos->getWorld()->getFolderName()) {
                $reg1 = $region->getPos1();
                $reg2 = $region->getPos2();
        ...
        if ($currentRegion == "") {
            if ($this->regionExists("global." . $pos->getWorld()->getFolderName())) {
                $currentRegion = "global." . $pos->getWorld()->getFolderName();
            }
        }
            ...
            if ($old !== "") {
                ...
                if ($old->getFlag("allowed-leave") === "false") {
                    if (!$player->hasPermission("worldguard.leave." . $oldregion)) {
                        $player->sendMessage(TextFormat::RED . $this->resourceManager->getMessages()["denied-leave"]);
                        return false;
                    }
                }
            ...
            if ($new !== "") {
                ...
                if ($new->getFlag("allowed-enter") === "false") {
                    if (!$player->hasPermission("worldguard.enter." . $newregion)) {
                        $player->sendMessage(TextFormat::RED . $this->resourceManager->getMessages()["denied-enter"]);
                        return false;
                    }
                }
    ...
    {
        $region = $this->players[$id = $player->getUniqueId()->toString()];
    ...
    public function processCreation(Player $player)
    {
        if (isset($this->creating[$id = $player->getUniqueId()->toString()], $this->process[$id])) {
            ...
            $permission = new Permission("worldguard.enter." . $name, "Allows player to enter the " . $name . " region.");
            $permission->addChild("worldguard.enter", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.leave." . $name, "Allows player to leave the " . $name . " region.");
            $permission->addChild("worldguard.leave", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.place." . $name, "Allows player to build blocks in " . $name . " region.");
            $permission->addChild("worldguard.place", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.block-place." . $name, "Allows player to build blocks in " . $name . " region.");
            $permission->addChild("worldguard.block-place", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.break." . $name, "Allows player to break blocks in " . $name . " region.");
            $permission->addChild("worldguard.break", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.block-break." . $name, "Allows player to break blocks in " . $name . " region.");
            $permission->addChild("worldguard.block-break", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.edit." . $name, "Allows player to edit blocks in " . $name . " region.");
            $permission->addChild("worldguard.edit", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.eat." . $name, "Allows player to eat in " . $name . " region.");
            $permission->addChild("worldguard.eat", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.drop." . $name, "Allows player to drop items in " . $name . " region.");
            $permission->addChild("worldguard.drop", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.usechest." . $name, "Allows player to use chests in " . $name . " region.");
            $permission->addChild("worldguard.usechest", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.usechestender." . $name, "Allows player to use ender chests in " . $name . " region.");
            $permission->addChild("worldguard.usechestender", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.usetrappedchest." . $name, "Allows player to use trapped chests in " . $name . " region.");
            $permission->addChild("worldguard.usetrappedchest", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.enchantingtable." . $name, "Allows player to use enchanting table in " . $name . " region.");
            $permission->addChild("worldguard.enchantingtable", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.usedoors." . $name, "Allows player to use doors in " . $name . " region.");
            $permission->addChild("worldguard.usedoors", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.usetrapdoors." . $name, "Allows player to use trapdoors in " . $name . " region.");
            $permission->addChild("worldguard.usetrapdoors", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.usegates." . $name, "Allows player to use gates in " . $name . " region.");
            $permission->addChild("worldguard.usegates", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.usefurnaces." . $name, "Allows player to use furnaces in " . $name . " region.");
            $permission->addChild("worldguard.usefurnaces", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.useanvil." . $name, "Allows player to use anvils in " . $name . " region.");
            $permission->addChild("worldguard.useanvil", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.usecauldron." . $name, "Allows player to use cauldron in " . $name . " region.");
            $permission->addChild("worldguard.usecauldron", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.usebrewingstand." . $name, "Allows player to use brewing stands in " . $name . " region.");
            $permission->addChild("worldguard.usebrewingstand", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.usebeacon." . $name, "Allows player to use beacons in " . $name . " region.");
            $permission->addChild("worldguard.usebeacon", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.usepressureplate." . $name, "Allows player to use pressureplates in " . $name . " region.");
            $permission->addChild("worldguard.usepressureplate", true);
            PermissionManager::getInstance()->addPermission($permission);

            $permission = new Permission("worldguard.usebutton." . $name, "Allows player to use buttons in " . $name . " region.");
            $permission->addChild("worldguard.usebutton", true);
            PermissionManager::getInstance()->addPermission($permission);
            $this->resourceManager->saveRegions($this->regions);
            return $name;
        }
    ...
    {
        if (!isset($args[1])) {
            if (strtolower($cmd->getName()) === "worldguard") {
                    ...
                    switch ($args[0]) {
                        case "setbiome":
                            ...
                            if (isset($args[1]) && isset($args[2])) {
                                if (!ctype_alnum($args[1])) {
                                    $issuer->sendMessage(TextFormat::RED . 'Region name must be alpha numeric.');
                                    return false;
                                }
                                if ($this->regionExists($args[1])) {
                                    Utils::setBiome($this, $this->getRegion($args[1]), $args[2]);
                                    $issuer->sendMessage(TextFormat::YELLOW . 'You have changed the region\'s biome.');
                                    $this->resourceManager->saveRegions($this->regions);
                                } else {
                                    $issuer->sendMessage(TextFormat::RED . $args[1] . ' region does not exist. Use /region list to get a list of all regions.');
                                }
                            } else {
                                $issuer->sendMessage(TextFormat::RED . '/region setbiome <name> <biome_name>');
                            }
                            break;
                        case "create":
                            ...
                            if (isset($args[1])) {
                                if (!ctype_alnum($args[1])) {
                                    $issuer->sendMessage(TextFormat::RED . 'Region names cannot contain special characters.');
                                    return false;
                                }
                                if ($this->regionExists($args[1])) {
                                    $issuer->sendMessage(TextFormat::RED . 'This region already exists. Redefine it using /region redefine ' . $args[1] . ', or remove it using /region remove ' . $args[1]);
                                    return false;
                                } else {
                                    if (isset($args[2])) {
                                        if ($args[2] == "extended") {
                                            ...
                                            $this->extended[$id] = [];
                                            $issuer->sendMessage(TextFormat::YELLOW . 'Right-Click two positions to complete creating the extended region (' . $args[1] . ').');
                                        } else {
                                            $issuer->sendMessage(TextFormat::RED . "Flag '" . $args[2] . "' not recognized.");
                                            return false;
                                        }
                                    } else {
                                        if ($args[1] == "global") {
                                            if ($this->regionExists($args[1] . "." . $issuer->getWorld()->getFolderName())) {
                                                $issuer->sendMessage(TextFormat::RED . "A global region for this world already exists!");
                                                return false;
                                            } else {
                                                unset($this->creating[$id = $issuer->getUniqueId()->toString()], $this->process[$id]);
                                                $this->process[$id] = ("global." . $issuer->getWorld()->getFolderName());
                                                $this->creating[$id][] = [0, 0, 0, $issuer->getWorld()->getFolderName()];
                                                $this->creating[$id][] = [0, 0, 0, $issuer->getWorld()->getFolderName()];
                                                $this->processCreation($issuer);
                                                $issuer->sendMessage(TextFormat::GREEN . "Global region for world " . $issuer->getWorld()->getFolderName() . " created.");
                                            }
                                        } else {
                                            unset($this->creating[$id = $issuer->getUniqueId()->toString()], $this->process[$id]);
                                            $this->creating[$id] = [];
                                            $this->process[$id] = $args[1];
                                            $issuer->sendMessage(TextFormat::YELLOW . 'Right-Click two positions to complete creating the region (' . $args[1] . ').');
                                        }
                                    }
                                }
                            } else {
                                $issuer->sendMessage(TextFormat::RED . '/region create <name>');
                            }
                            break;
                        case "delete":
                            ...
                            if (isset($args[1])) {
                                if ($this->regionExists($args[1])) {
                                    ...
                                    $issuer->sendMessage(TextFormat::YELLOW . 'You have deleted the region: ' . $args[1]);
                                    $this->resourceManager->saveRegions($this->regions);
                                } else {
                                    $issuer->sendMessage(TextFormat::RED . $args[1] . ' region does not exist. Use /region list to get a list of all regions.');
                                }
                            } else {
                                $issuer->sendMessage(TextFormat::RED . '/region delete <name>');
                            }
                            break;
                        case "list":
                            $msg = TextFormat::LIGHT_PURPLE . "Regions: \n" . TextFormat::LIGHT_PURPLE;
                            if (empty($this->regions)) {
                                $msg .= "You haven't created any regions yet. Use /region create <name> to create your first region.";
                            } else {
                                $msg .= implode(TextFormat::WHITE . ', ' . TextFormat::LIGHT_PURPLE, array_keys($this->regions));
                            }
                        ...
                        case "info":
                            ...
                            if ($reg !== "") {
                                ...
                                $issuer->sendMessage(TextFormat::RED . "You are not currently standing in any regions.");
                                return false;
                            }
                            break;
                        case "redefine":
                            if (!isset($args[1])) {
                                $issuer->sendMessage(TextFormat::RED . '/region redefine <region>');
                                return false;
                            } else {
                                if (!$this->regionExists($args[1])) {
                                    $issuer->sendMessage(TextFormat::RED . $args[1] . ' region does not exist. Use /region list to get a list of all regions.');
                                    ...
                                    $this->creating[$id] = [];
                                    $this->process[$id] = $args[1];
                                    $issuer->sendMessage(TextFormat::LIGHT_PURPLE . 'Right-Click two positions to redefine your region (' . $args[1] . ').');
                                }
                            }
                            break;
                        case "getplayer":
                            if (isset($args[1])) {
                                if (($player = $this->getServer()->getPlayerExact($args[1])) !== null) {
                                    $reg = $this->getRegionOf($player);
                                    if ($reg !== "") {
                                        $issuer->sendMessage(TextFormat::YELLOW . $player->getName() . ' is in ' . $reg . '.');
                                    } else {
                                        $issuer->sendMessage(TextFormat::YELLOW . $player->getName() . 'is not in any region.');
                                    }
                                } else {
                                    $issuer->sendMessage(TextFormat::RED . $args[1] . ' is offline.');
                                }
                            } else {
                                $issuer->sendMessage(TextFormat::RED . '/region getplayer <player>');
                            }
                            break;
                        case "flag":
                        case "flags":
                            ...
                            if (isset($args[1], $args[2])) {
                                if (!$this->regionExists($args[2])) {
                                    $issuer->sendMessage(TextFormat::RED . 'The specified region does not exist. Use /region list to get a list of all regions.');
                                    return false;
                                }
                                if ($args[1] !== "get") {
                                    if (!isset($args[3])) {
                                        $issuer->sendMessage(TextFormat::RED . "You haven't specified the <flag>.");
                                        return false;
                                    } elseif (!$this->flagExists($args[3])) {
                                        $issuer->sendMessage(TextFormat::RED . "The specified flag does not exist. Available flags:\n" . TextFormat::LIGHT_PURPLE . implode(TextFormat::WHITE . ', ' . TextFormat::LIGHT_PURPLE, array_keys(self::FLAGS)));
                                        return false;
                                    }
                                }
                                switch ($args[1]) {
                                    case "get":
                                        ...
                                        $issuer->sendMessage(TextFormat::LIGHT_PURPLE . $args[2] . "'s flags:\n" . $flags);
                                        break;
                                    case "set":
                                        if (!isset($args[4])) {
                                            $issuer->sendMessage(TextFormat::RED . 'You must specify the <value> of the flag.');
                                            return false;
                                        }
                                        ...
                                        if ($opt !== null) {
                                            ...
                                            $issuer->sendMessage(TextFormat::YELLOW . 'Flag has been updated successfully.');
                                            $this->resourceManager->saveRegions($this->regions);
                                        }
                                        break;
                                    case "reset":
                                        $this->getRegion($args[2])->resetFlag($args[3]);
                                        $issuer->sendMessage(TextFormat::YELLOW . "Flag " . $args[3] . " has been reset to it's default value.");
                                        $this->resourceManager->saveRegions($this->regions);
                                        break;
                                }
                            } else {
                                $issuer->sendMessage(TextFormat::RED . "/region flags <get/set/reset> <region> <flag> <value>\n" . TextFormat::GRAY . '<value> argument is only needed if you are setting the flag.');
                            }
                            break;
                    }
                } else {
                    $issuer->sendMessage(implode("\n" . TextFormat::LIGHT_PURPLE, [
                            "§9§lWorldGuard ("]) . $this->getServer()->getVersion() . implode("\n" . TextFormat::LIGHT_PURPLE, [") §r§9Help Page §7(by Chalapa)",
                            " ",
